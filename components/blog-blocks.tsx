  ...
  return (
    <div className="w-full rounded-xl overflow-hidden bg-surface/60 backdrop-blur-md border border-white/6" style={{ minHeight: 420 }}>
      ...
      <div className="flex h-[80vh]">
        {/* file list (left) */}
  <aside className="relative w-64 bg-[#0b0f14] border-r border-white/6 p-3 overflow-auto">
            ...
            {posts.map((p) => (
              <li key={p.slug}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-md hover:bg-white/3 ${active === p.slug ? 'bg-white/5 ring-1 ring-white/10' : ''}`}
                >
                  {p.title}
                </button>
              </li>
          ...
          <div className="absolute left-3 right-3 bottom-3">
            <div className="px-3 py-2 rounded-md text-xs text-white/90 bg-gradient-to-r from-white/4 to-black/20 backdrop-blur-sm border border-white/6 shadow-sm text-center">
        ...
        {/* editor/viewer (right) */}
        <main className="flex-1 p-6 overflow-auto" style={{ background: 'linear-gradient(180deg, rgba(10,12,15,0.5), rgba(6,8,10,0.6))' }}>
          {active ? (
                <article className="prose prose-invert dark:prose-invert max-w-none">
                  {postTitle ? <h1 className="text-2xl font-bold mb-4">{postTitle}</h1> : null}
                  {postDate ? <div className="text-sm text-muted-foreground mb-4">{postDate}</div> : null}
